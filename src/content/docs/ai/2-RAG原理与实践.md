...
这篇文章做一件事。把 RAG 链路里的常见技术、方案差异和选型依据放到一张图里。需要落地代码时，可以继续看 [一个可落地的 RAG 知识库组件实战](/ai/rag-knowledge-base-component/)。那篇文章用 FastAPI、LlamaIndex 和 pgvector 走完整个实现过程，这一篇负责解释为什么会有这些选择，还有哪些路可以走。
...
[从 Demo 到服务的 RAG 知识库组件实战](/ai/rag-knowledge-base-component/) 展开了一条具体落地路线。它包括 FastAPI 服务、LlamaIndex 编排、pgvector、多知识库隔离、异步索引、国产模型接入、部署和已知限制。阅读时可以用本文的选型框架继续追问。
