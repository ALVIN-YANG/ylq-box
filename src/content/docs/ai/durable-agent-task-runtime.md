---
title: "长任务 Agent Runtime：状态机、Checkpoint、恢复、取消与终态保护"
description: "把需要几分钟到几小时的 Agent 任务设计成可恢复、可取消、可审计的持久化执行系统。"
date: 2026-07-31
...
只要任务可能跨越一次 HTTP 请求，它就不应该依赖某个进程内的 `while` 循环活到最后。
...
它们面对的不是“模型能不能推理”，而是进程重启、重复消息、超时、取消、资源竞争和部分成功。
